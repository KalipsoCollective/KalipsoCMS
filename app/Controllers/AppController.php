		...
		if (Base::config('app.dev_mode')) {
			...
			switch ($action) {
				...
				case 'check_languages':
					$head = Base::lang('base.check_languages');
					$title = $head . ' | ' . $title;
					$description = Base::lang('base.check_languages_message');


					$languagefiles = glob(Base::path('app/Resources/localization/*.php'));
					$languages = [];
					foreach ($languagefiles as $languagefile) {
						$require = require $languagefile;
						$languages[basename($languagefile, '.php')] = Base::arrayFlat($require);
					}
					...
					if (count($diff)) {
						...
						<div class="table-responsive">
							<table class="table table-dark table-sm table-hover table-striped">
								<thead>
									<tr>
										<th scope="col">'.Base::lang('base.language').'</th>
										<th scope="col">'.Base::lang('base.data').'</th>
									</tr>
								</thead>
								<tbody>';
						...
						$cols = '<p class="text-success">' . Base::lang('base.no_missing_definitions.') . '</p>';
					}
