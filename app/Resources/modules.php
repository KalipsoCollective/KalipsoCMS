...
return [
	'pages' => [
		...
		'inputs' => [
			...
			'description' => [
				...
				'attributes' => [
					'required' => 'true',
					'data-kn-input' => Base::base('/management/pages/autocomplete'),
					'data-kn-autocomplete' => json_encode(['field' => 'description']),
				],
			],
			'content' => [
	...
	'blog' => [
		...
		'from' => '(SELECT 
						...
						REPLACE(
							REPLACE(
								REPLACE(
									REPLACE(
										IFNULL(JSON_EXTRACT(x.input, \'$.single_image\'), ""),
										" ",
										""
									),
									"\"",
						...
						(SELECT JSON_ARRAYAGG(files) AS files FROM files WHERE FIND_IN_SET(id, single_image)) AS single_image_src,
		...
		'table' => [
			...
			'single_image_src' => [
				'formatter' => function($row) {
					$return = '';
					if ($row->single_image_src AND $srcset = @json_decode($row->single_image_src)) {
						$return = '<div class="image-group">';
						foreach ($srcset as $src) {
							$hrefDir = 'upload/' . $src->original;
							$srcDir = 'upload/' . (isset($src->sm) !== false ? $src->sm : $src->original);
		...
		'columns' => [
			...
			[
				"searchable" => false,
				"orderable" => false,
				"title" => Base::lang('base.single_image'),
				"key" => "single_image_src"
			],
			[
				"searchable" => [
					"type" => "date",
	...
	'sliders' => [
		...
		'from' => '(SELECT 
						...
						IFNULL(JSON_UNQUOTE(JSON_EXTRACT(x.input, \'$.title.'.Base::lang('lang.code').'\')), "-") AS title,
						IFNULL(JSON_UNQUOTE(JSON_EXTRACT(x.input, \'$.content.'.Base::lang('lang.code').'\')), "-") AS content,
						IFNULL(JSON_UNQUOTE(JSON_EXTRACT(x.input, \'$.link\')), "-") AS link,
						IFNULL(JSON_UNQUOTE(JSON_EXTRACT(x.input, \'$.order\')), "0") AS order_num,
						REPLACE(
							REPLACE(
								REPLACE(
									REPLACE(
						...
						FROM_UNIXTIME(x.created_at, "%Y.%m.%d %H:%i") AS created,
						IFNULL(FROM_UNIXTIME(x.updated_at, "%Y.%m.%d"), "-") AS updated
					FROM `contents` x WHERE x.module = "sliders") AS raw',
		'table' => [
			'id' => [
				'primary' => true,
			],
			...
			'content' => [
				'formatter' => function($row) {
					...
					$content = Base::stringShortener($row->content, 100);
					return $content == '' ? '-' : $content;
				}
			],
			'link' => [
